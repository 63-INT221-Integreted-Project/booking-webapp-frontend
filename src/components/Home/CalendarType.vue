...
async function fetchEvents() {
  try {
    if(userStore.isAdmin() || userStore.isStudent() || userStore.isLecturer()){
      ...
      dayjs
        ...
        events.value = await EventService.findAllByBetweenDateGuest(
      dayjs
        .utc(new Date(year.value, month.value, no_of_days.value.length))
        .date(1)
        .hour(0)
    ...
    if (
      ...
      if (userStore.isAdmin() || userStore.isStudent() || userStore.isLecturer()) {
        ...
        let eventToday = await EventService.findAllByBetweenDateGuest(
          dayjs.utc().format("YYYY-MM-DDT[00:00:00Z]"),
          dayjs.utc().format("YYYY-MM-DDT[23:59:59Z]")
        );
        events.value.push(...eventToday);
  ...
  <div class="grid grid-rows-3 grid-flow-col gap-x-12 gap-y-0 mt-4">
        ...
        <div class="w-full text-gray-800 flex justify-start">
          <div class="flex flex-col w-full pr-6">
            <div class="flex flex-wrap justify-between">
              <h3 class="font-semibold text-lg leading-tight truncate mb-4">
    ...
    <div class="col-span-3">
      ...
      >
        ...
        <!-- content -->
        <div class="w-full text-gray-800 flex items-center justify-end px-12">
          <div class="text-right flex flex-col">
            <h3 class="text-lg leading-tight truncate">จองภายในเดือนนี้</h3>
    ...
    <div class="col-span-5 row-span-3">
      <div
        ...
        :class="{
          'justify-between':
            userStore.getUserRole() && userStore.getUserRole() !== 'lecturer',
          'justify-end':
            !userStore.getUserRole() || userStore.getUserRole() === 'lecturer',
        }"
      >
            ...
            <div class="flex flex-wrap border-t border-l">
              ...
              >
                ...
                >
                  ...
                  ></div>
                  <div style="height: 120px" class="overflow-y-auto mt-1">
                    <!-- <div 
										class="absolute top-0 right-0 mt-2 mr-2 inline-flex items-center justify-center rounded-full text-sm w-6 h-6 bg-gray-700 text-white leading-none"
										x-show="events.filter(e => e.event_date === new Date(year, month, date).toDateString()).length"
										x-text="events.filter(e => e.event_date === new Date(year, month, date).toDateString()).length"></div> -->
                    <template v-for="(event, index) in getCardEvents(date)">
                      <div
                        ...
                        v-if="
                          index === 0 ||
                          (index === 1 && getCardEvents(date).length === 2)
                        "
                      >
                        <p
                          v-html="formatBookingName(event.bookingName)"
                      ...
                      <div
                        ...
                        "
                        v-else-if="
                          index === 1 && getCardEvents(date).length >= 3
                        "
                      >
